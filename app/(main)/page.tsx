...
import { AgentCTA } from "@/components/cta/AgentCTA";
...
export default async function HomePage() {
  ...
  const recentResult = hasFeatured
    ? null
    : await sanityFetch({ query: RECENT_PROPERTIES_QUERY });
  const showProperties = hasFeatured
    ? (featuredProperties ?? [])
    : (recentResult?.data ?? []);
  const isFallback = !hasFeatured;
  ...
  return (
    <div>
      ...
      <section className="py-20 md:py-28">
        <div className="container">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10">
            <div>
              <h2 className="text-3xl md:text-4xl font-bold font-heading">
                {isFallback ? "Featured & Recent" : "Featured Properties"}
              </h2>
              <p className="text-muted-foreground mt-2">
                {isFallback
                  ? "Recent active listings while we curate more featured homes"
          ...
          {showProperties.length > 0 ? (
            <PropertyGrid properties={showProperties} />
          ) : (
      ...
      {/* CTA - Become an Agent (gated by Clerk Show / plan) */}
      <AgentCTA />
    </div>
  );
}
